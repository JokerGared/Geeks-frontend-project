...
const MobileMenu = ({ isOpen, onCloseMobileMenu }) => {
  ...
  return (
    <div className={clsx(s.mobileBackdrop, isOpen && s.isOpen)} onClick={handleBackDropClick}>
      <div className={clsx(s.mobileMenu)}>
        <div className={clsx(s.logoCloseButtonContainer)}>
          ...
          <div className={clsx(s.tabCreateCloseContainer)}>
            {isLoggedIn ? (
              <NavLink to='/create' className={clsx(s.tabCreateLink)} onClick={onCloseMobileMenu}>
                Create an article
              </NavLink>
            ) : (
              <NavLink to='/register' className={clsx(s.tabJoinLink)} onClick={onCloseMobileMenu}>
                Join now
              </NavLink>
            )}
            <button
            ...
            >
              <svg width={32} height={32} className={clsx(s.closeIcon)} aria-hidden='true'>
        ...
        <div className={clsx(s.mobNavLinkContainer)}>
          <NavLink to='/' className={handleActiveClass} onClick={onCloseMobileMenu}>
            Home
          </NavLink>
          <NavLink to='/articles' className={handleActiveClass} onClick={onCloseMobileMenu}>
            Articles
          </NavLink>
          <NavLink to='/authors' className={handleActiveClass} onClick={onCloseMobileMenu} end>
            Creators
          </NavLink>
          ...
          {isLoggedIn ? (
            ...
            <>
              <NavLink to='/login' className={handleActiveClass} onClick={onCloseMobileMenu}>
                Log in
              </NavLink>
              <NavLink to='/register' className={clsx(s.mobJoinLink)} onClick={onCloseMobileMenu}>
                Join now
              </NavLink>
            </>
          )}
